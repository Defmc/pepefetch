...
fn format_login(hide_infos: bool, usr: &str, host: &str) -> String {
    ...
    format!(" {}@{}", Red.bold().paint(usr), Red.bold().paint(host))
}
...
fn main() {
    ...
    let hide_infos = env::args().any(|arg| arg == "--hide" || arg == "-h");
    ...
    let get_env_var = |var: &str| {
        env::var(var).unwrap_or_else(|_| format!("Variable {var} not defined", var = var))
    };

    let mut ascii_img: Vec<String> = include!("../assets/ascii")
        .split('\n')
        .map(|s| s.to_owned())
        .collect();
    ...
    let infos = vec![
        ...
        format_env(
            ...
            "Uptime",
            format!(
                "{up_hours} hours, {up_minutes} mins",
                up_hours = up_hours,
        ...
        format_env(
            hi,
            "Memory",
            format!("{used}MiB / {total}MiB", used = used, total = total),
        ),
    ];
